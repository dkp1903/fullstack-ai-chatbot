        ...
        if response:
            for stream, messages in response:
                ...
                for message in messages:
                    ...
                    cache.add_message_to_cache(token=token, source="human", message_data=msg.dict())
                    ...
                    cache.add_message_to_cache(token=token, source="bot", message_data=msg.dict())

                # Delete messaage from queue after it has been processed

                consumer.delete_message(stream_channel="message_channel", message_id=message_id)
